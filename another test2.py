...
def make_label(parent, text, row, column, columnspan=1):
    label = tk.Label(
        parent,
        text=text,
        bg=PANEL_COLOR,
...
def make_entry(parent, row, column, width=25):
    entry = tk.Entry(
        parent,
        width=width,
        bg=ENTRY_COLOR,
...
def make_button(parent, text, command, row, column, danger=False, width=18):
    button = tk.Button(
        parent,
        text=text,
        command=command,
...
def make_listbox(parent, row, column, height=7, columnspan=4):
    listbox = tk.Listbox(
        parent,
        width=90,
        height=height,
    ...
    listbox.grid(row=row, column=column, columnspan=columnspan, padx=5, pady=10, sticky="w")
    return listbox
...
tk.Label(
    dashboard_page,
    text="Use the menu on the left to manage student records, class schedules, and attendance.",
    bg=BG_COLOR,
    fg=SUBTEXT_COLOR,
    font=FONT_MAIN,
    wraplength=700,
...
dashboard_panel.pack(fill="x", pady=20)

tk.Label(
    dashboard_panel,
    text=(
        "• Record attendance per student and schedule/class\n"
        "• Prevent duplicate attendance records per class per day\n"
        "• Add, edit, and delete student records\n"
        "• Add, edit, and delete schedules\n"
        "• Detect schedule conflicts and adjust based on priority"
    ),
    bg=PANEL_COLOR,
    fg=TEXT_COLOR,
    font=FONT_MAIN,
...
student_count_label = tk.Label(stats_panel, text="Students: 0", bg=PANEL_COLOR, fg=TEXT_COLOR, font=FONT_MAIN)
student_count_label.pack(anchor="w", pady=3)

schedule_count_label = tk.Label(stats_panel, text="Schedules: 0", bg=PANEL_COLOR, fg=TEXT_COLOR, font=FONT_MAIN)
schedule_count_label.pack(anchor="w", pady=3)

attendance_count_label = tk.Label(stats_panel, text="Attendance Records: 0", bg=PANEL_COLOR, fg=TEXT_COLOR, font=FONT_MAIN)
attendance_count_label.pack(anchor="w", pady=3)

# =====================================================
# STUDENT PAGE
# =====================================================

student_panel = make_panel(student_page, "Student Management")
...
student_list = make_listbox(student_panel, 1, 0, height=7, columnspan=4)
...
schedule_panel = make_panel(schedule_page, "Schedule Management")
...
schedule_list = make_listbox(schedule_panel, 6, 0, height=6, columnspan=4)
...
attendance_panel = make_panel(attendance_page, "Attendance")
...
attendance_list = make_listbox(attendance_panel, 2, 0, height=8, columnspan=5)
